...
function UpdateUser() {
    ...
    const handleSubmit = useCallback(async (e?: FormEvent) => {
        ...
        try{
            ...
            const res = await api.put<RegisterFormData, BasicResultSet>("/users/update", { userId: `${ location.state.user.id }` }, formData);
            if(res){
                console.log(res);
                alert.setSuccess("User updated successfully.");
            }
        }catch(err){
            console.log(err);
                ...
                <Container sx={{
                    ...
                    >
                        <Box sx={{
                            ...
                            <DatePicker value={moment(formData.birthday)} name="birthday" label="Birthday" format="YYYY-MM-DD" onChange={handleTimeInput} />
